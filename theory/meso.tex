...
%=========================================================
    ...
    %=========================================================
        ...
        To quantify the Andreev reflection processes introduced above, we employ the Blonder--Tinkham--Klapwijk (BTK) model. BTK provides a scattering description of an N--S interface with arbitrary transparency and allows calculating the energy-resolved probabilities for normal reflection, Andreev reflection, and transmission into the superconductor.

        To account for finite quasi-particle lifetimes, we use the phenomenological modification introduced by Pleceník et al., replacing the quasi-particle energy by a complex quantity $E \rightarrow |E| + \ima\gamma$. The resulting coherence factors ($u_0$, $v_0$), normalisation factor ($d$), the dimensionless barrier strength ($Z$), the Andreev ($A$) and normal reflection probability ($B$), are given by
        \begin{equation}
            \begin{array}{ll}
        ...
        For subgap energies the transmission into the superconductor vanishes, and the probabilities satisfy $A(E) + B(E) = 1$. It is therefore convenient to define the energy-resolved spectral weights of the single-particle (1e) and two-particle (2e)     processes,
        \begin{equation}
            \rho_\mathrm{1e}(E) = 1 - A(E) - B(E)\,, \quad
            \rho_\mathrm{2e}(E) = 2 \cdot A(E) \,,
            \label{eq:meso:btk-dos}
        \end{equation}
        which determine the relative contributions of normal and Andreev processes to charge transport, as shown in Figure~\ref{fig:meso:btk-dos}. Although these functions resemble densities of states, they should not be interpreted as the physical BCS DOS. Instead, they are the BTK spectral kernels that enter the current formula. 
        \begin{figure}[t]
            \centering
        ...
        The current through an N-S interface follows directly from the BTK kernel,
        \begin{equation}
        ...
        \end{equation}
        and yields the characteristic \textit{I--V} and \textit{dI--dV} curves shown in Fig.~\ref{fig:meso:btk-iv}. As the transparency increases, the Andreev contribution grows, enhancing the subgap conductance and reducing the height of the coherence peaks. In the tunneling limit ($\tau\ll1$), the expression reduces to the usual quasi-particle tunneling formula, while for $\tau\approx1$ it approaches the Andreev limit where charge is transferred predominantly in units of $2e$.
        \begin{figure}[t]
            \centering
        ...
        % With these quantities the spectral weight $\rho$ of the 1e and 2e process can be expressed as
        % \begin{equation}
        %     \rho_\mathrm{1e}(E) = 1 - A(E) - B(E)\,, \quad
        %     \rho_\mathrm{2e}(V) = 2 \cdot A(E) \,,
        %     \label{eq:meso:btk-dos}
        % \end{equation}
        % and are shown in Figure~\ref{fig:meso:btk-dos}. Normalized by the transmission those can be threaded as effective DOS.
        % \begin{figure}[t]
        %     \centering
        %     \import{theory/meso}{btk-dos.pgf}
        %     \caption{platzhalter}
        ...
        % The total current through an NS contact, is given by
        % \begin{equation}
        %     I_\mathrm{NS}(V) = \frac{G_0}{e} \int_{-\infty}^{\infty} \left( \rho_\mathrm{1e}(V) + \rho_\mathrm{2e}(V) \right) \left(f(E) - f(E + eV)\right) \mathrm{d}E\,.
        %     \label{eq:meso:btk-iv}
        % \end{equation}
        % Figure~\ref{fig:meso:btk-iv} illustrates the resulting \textit{I-V} and \textit{dI-dV} characteristics obtained from the analytical BTK formulation, showing how the subgap conductance and coherence peaks evolves with interface transparency.
        % \begin{figure}[t]
        %     \centering
        %     \import{theory/meso}{btk-current.pgf}
        %     \caption{
