...
\subsection{DC Josephson Relation}
The Josephson effect describes the coherent tunneling of Cooper pairs between two superconductors separated by a weak link (insulator, normal metal, or constriction). In the tunneling limit the supercurrent is governed by the phase difference $\varphi=\varphi_1-\varphi_2$ of the macroscopic order parameters, leading to the current–phase relation
\begin{equation}
    I_s(\varphi) = I_c \sin\varphi.
\end{equation}
Here $I_c$ is the \emph{critical current}, determined by the microscopic transparency of the junction and the superconducting gap. For an ideal, uniform tunnel junction at $T=0$ the Ambegaokar--Baratoff result provides a material-specific constraint,
\begin{equation}
    I_c R_N = \frac{\pi \Delta}{2e},
\end{equation}
where $R_N$ is the normal-state resistance measured at $|eV|\!\gg\!2\Delta$. At finite temperature $I_c$ is reduced approximately as $I_c(T)\!\propto\!\Delta(T)\tanh\!\big[\Delta(T)/(2k_\mathrm{B}T)\big]$.

\subsubsection{When to expect a measurable DC supercurrent.}
A dissipationless branch at $V=0$ is observed only if the Josephson energy
\begin{equation}
    E_J = \frac{\hbar I_c}{2e}
\end{equation}
exceeds thermal and environmental fluctuations. A practical criterion is $E_J \gg k_\mathrm{B}T$ and a sufficiently low-noise, well-filtered electromagnetic environment. In the resistively and capacitively shunted junction (RCSJ) picture, overdamping or strong noise leads to \emph{phase diffusion}: the zero-voltage state is washed out even though Josephson coupling is present. Small junction capacitance (large charging energy $E_C=e^2/2C$) can further suppress the DC branch when $E_C \gg \approx  E_J$.

\subsubsection{Geometrical and materials conditions.}
A DC Josephson current requires two superconducting electrodes with established phase coherence and a barrier that permits pair tunneling (insulating or weak metallic link). If one electrode is normal (S--I/N) or if pair breaking by magnetic field, magnetic impurities, or excess quasiparticles is strong, the DC Josephson effect is suppressed, although subgap quasiparticle transport can remain.

\subsection{AC Josephson Effect and Shapiro Steps}
Applying a constant voltage $V$ across a Josephson junction imposes a linear-in-time phase evolution according to the second Josephson relation
\begin{equation}
    \frac{d\varphi}{dt} = \frac{2eV}{\hbar}\quad\Rightarrow\quad 
    \varphi(t)=\varphi_0+\frac{2eV}{\hbar}t.
\end{equation}
The supercurrent therefore oscillates at the \emph{Josephson frequency}
\begin{equation}
    f_J = \frac{1}{2\pi}\frac{d\varphi}{dt}=\frac{2e}{h}\,V,
\end{equation}
\begin{equation}
    I_s(t)=I_c\sin\!\left(\varphi_0+\frac{2eV}{\hbar}t\right).
\end{equation}
When the junction is irradiated with a monochromatic microwave of angular frequency $\omega$, the phase is additionally modulated. Phase locking between the intrinsic Josephson oscillation and the drive leads to quantized constant-voltage plateaus, the \emph{Shapiro steps}, at
\begin{equation}
    V_n = \frac{n\,h f}{2e}=\frac{n\,\hbar\omega}{2e},\qquad n \in \mathrm{Z}.
\end{equation}
The width of step $n$ scales with Bessel functions of the drive amplitude (in the RCSJ model with an AC current bias), providing a characteristic dependence on microwave power. Clear Shapiro steps are a robust signature of a phase-coherent Josephson junction.

\subsubsection{Conditions for observing Shapiro steps.}
Shapiro steps require a non-negligible $I_c$ (so that $E_J$ is not too small), adequate coupling of the microwave to the junction, and an electromagnetic environment that allows phase locking (not excessively noisy or overdamped). Strong thermal fluctuations or dominant quasiparticle dissipation can reduce step visibility or smear them out.

\subsection{Coherent vs Incoherent Tunneling under Microwaves}
Microwave irradiation of weak links can produce superficially similar voltage features via two distinct mechanisms: \emph{coherent} phase dynamics of the Josephson effect (Shapiro physics) and \emph{incoherent} photon-assisted tunneling of quasiparticles. Disentangling them is crucial for interpretation.

\subsubsection{Coherent (Josephson) response.}
Here the AC field modulates the superconducting phase difference. The junction supports a supercurrent at $V=0$, and under RF drive exhibits Shapiro steps at $V_n=n\hbar\omega/2e$. The response is governed by the current–phase relation and the RCSJ dynamics (capacitance, damping, noise). Spectral signatures: quantized constant-voltage plateaus; step amplitudes follow Bessel-like power dependence; features are centered at exact multiples of $\hbar\omega/2e$ independent of gap edges.

\subsubsection{Incoherent photon-assisted tunneling (Tien--Gordon framework).}
In the tunneling (or strongly overdamped) regime where coherent supercurrent is negligible, the microwave acts as a classical voltage modulation $V(t)=V_\mathrm{dc}+V_\mathrm{ac}\cos\omega t$. The time-averaged current is
\begin{equation}
    I(V_\mathrm{dc}) = \sum_{n=-\infty}^{\infty} J_n^2\!\left(\alpha\right)\,
    I_0\!\left(V_\mathrm{dc} + \frac{n\hbar\omega}{e}\right),\qquad 
    \alpha=\frac{eV_\mathrm{ac}}{\hbar\omega},
\end{equation}
with $I_0(V)$ the dark $I$--$V$. Sidebands in $dI/dV$ appear as replicas of intrinsic spectral features (e.g.\ coherence peaks) shifted by $\pm n\hbar\omega/e$. Spectral signatures: satellite peaks around gap-related structures; no quantized constant-voltage plateaus at $\hbar\omega/2e$ unless phase coherence is present.

\subsubsection{Practical discrimination in experiments.}
\begin{itemize}
    \item \textbf{Zero-bias behavior:} A resolvable DC supercurrent (or its modulation with RF) indicates coherent Josephson coupling; purely resistive zero-bias with sidebands points to Tien--Gordon physics.
    \item \textbf{Feature location:} Shapiro plateaus occur at $V_n=n\hbar\omega/2e$ regardless of $\Delta$; Tien--Gordon replicas are offset from existing spectral features (e.g.\ near $\pm2\Delta/e$).
    \item \textbf{Power dependence:} Shapiro step heights follow Bessel functions of \emph{phase (or AC current)}; Tien--Gordon sidebands follow Bessel weights of the \emph{voltage modulation} acting on $I_0(V)$.
    \item \textbf{Environmental sensitivity:} Phase locking (Shapiro) is sensitive to damping and noise (RCSJ parameters); Tien--Gordon sidebands persist even in overdamped, incoherent junctions.
\end{itemize}
In the context of this thesis, tunnel-barrier junctions are well described by the Tien--Gordon framework with suppressed $I_c$, whereas atomic contacts at higher transmission can exhibit Josephson coherence (Shapiro-like features) or, when incoherent pair tunneling dominates, show microwave-induced structures without true phase locking.
...
\subsection{Voltage Modulation and Energy Sidebands}
When a tunnel junction is irradiated with microwaves or any oscillating electric field, the voltage across the junction becomes time dependent,
\begin{equation}
    V(t) = V_\mathrm{dc} + V_\mathrm{ac}\cos(\omega t),
\end{equation}
where $V_\mathrm{dc}$ is the static bias, $V_\mathrm{ac}$ the amplitude of the alternating field, and $\omega = 2\pi f$ the angular frequency of the drive. In the simplest description, the AC voltage modulates the energy of electrons tunneling through the barrier by $\pm n\hbar\omega$, corresponding to the absorption ($n>0$) or emission ($n<0$) of $n$ photons. This periodic modulation leads to the appearance of discrete \emph{energy sidebands} in the tunneling spectrum.

Tien and Gordon \cite{tien_multiphoton_1963} treated this problem semiclassically by introducing a time-dependent phase factor in the tunneling Hamiltonian,
\begin{equation}
    H_T(t) = \sum_{k,q} T_{kq}\, c_{k}^\dagger c_{q}\, e^{i\frac{e}{\hbar}\int^t V(t')\, dt'},
\end{equation}
and expanding the exponential modulation in terms of Bessel functions:
\begin{equation}
    e^{i\alpha\sin(\omega t)} = \sum_{n=-\infty}^{+\infty} J_n(\alpha) e^{in\omega t}, 
    \qquad \alpha = \frac{eV_\mathrm{ac}}{\hbar\omega}.
\end{equation}
This expansion reveals that the AC field effectively mixes the electronic states of energy $E$ with those of $E + n\hbar\omega$, each weighted by a probability $J_n^2(\alpha)$ corresponding to an $n$‑photon process. The resulting time-averaged current becomes
\begin{equation}
    I(V_\mathrm{dc}) = \sum_{n=-\infty}^{+\infty} J_n^2(\alpha)\,
    I_0\!\left(V_\mathrm{dc} + \frac{n\hbar\omega}{e}\right),
\end{equation}
where $I_0(V)$ denotes the static $I$--$V$ characteristic in the absence of irradiation. The Bessel-function weighting encodes the intensity of each photon-assisted channel: for small amplitudes ($\alpha \ll 1$) only the $n = 0, \pm1$ terms contribute, while larger $\alpha$ values yield multiple visible sidebands.

This formalism naturally explains the appearance of additional conductance peaks in irradiated superconducting junctions: each original spectral feature (such as the coherence peaks at $\pm2\Delta/e$) is replicated at shifted voltages $V = V_0 \pm n\hbar\omega/e$, corresponding to the absorption or emission of $n$ photons. The strength of these replicas provides a direct measure of the effective AC voltage amplitude across the junction.

\subsection{Applicability and Limitations}
The Tien--Gordon model is strictly valid in the regime where the AC field acts as a classical perturbation and the junction responds incoherently—that is, when no long-lived phase coherence is established across the barrier. This condition corresponds to the tunneling limit of weak coupling ($\tau_i \ll 1$), where Cooper-pair tunneling is negligible and the current is dominated by independent quasiparticle processes. In this case, the microwave field merely modulates the potential difference and redistributes quasiparticle tunneling probabilities among discrete photon sidebands.

The model neglects self-consistent feedback between current and phase dynamics and assumes that the AC voltage is spatially uniform across the barrier. It therefore cannot capture phenomena that depend on coherent phase evolution, such as the DC or AC Josephson effect, or more generally, interference between multiple Andreev reflections at high transparency. For these situations, more elaborate treatments based on the time-dependent Bogoliubov--de Gennes or scattering approaches are required.

Despite these simplifications, the Tien--Gordon approach remains remarkably accurate for superconducting tunnel junctions and has become the standard framework for analyzing photon-assisted tunneling spectroscopy. In this thesis, it provides the theoretical basis for interpreting the microwave response of the aluminum tunnel-barrier junctions, as presented in Chapter~\ref{ch:tunnelbarrier}, and serves as the incoherent counterpart to the phase-coherent dynamics discussed in Chapter~\ref{sec:josephson}.

\subsection{Comparison to AC Josephson Effect}
It is instructive to contrast photon-assisted tunneling in the Tien--Gordon sense with the AC Josephson effect described earlier. In a Josephson junction supporting a coherent supercurrent, the phase difference $\varphi$ across the barrier evolves according to $\dot{\varphi}=2eV/\hbar$, leading to a periodic modulation of the supercurrent $I_s(t)=I_c\sin[\varphi(t)]$. When an external microwave drive is applied, this intrinsic oscillation can lock to the drive frequency, giving rise to quantized voltage plateaus—the Shapiro steps—at
\begin{equation}
    V_n = \frac{n\hbar\omega}{2e}.
\end{equation}
These plateaus are a manifestation of coherent phase locking between the internal Josephson oscillation and the external drive.

In contrast, within the Tien--Gordon regime, no coherent phase relation exists between the superconducting electrodes. The microwaves simply add an oscillatory voltage across the barrier, producing sidebands in the quasiparticle spectrum but no quantized voltage steps. The key distinction is therefore one of \emph{coherence}: Shapiro steps emerge from phase-coherent pair tunneling, whereas Tien--Gordon sidebands arise from incoherent single-particle tunneling under a modulated potential.

Experimentally, this difference manifests clearly in the low-bias behavior. A junction exhibiting a supercurrent and Shapiro steps displays zero-voltage transport and discrete voltage plateaus that scale with $\hbar\omega/2e$, independent of the superconducting gap. In contrast, a purely tunneling junction shows suppressed current near zero bias and sideband replicas near the gap edges that shift with photon energy $\hbar\omega$. The simultaneous observation of both behaviors—e.g., weak Shapiro-like modulation superimposed on photon-assisted sidebands—signals an intermediate regime where pair and quasiparticle tunneling coexist, as explored in the high-transmission measurements presented later in this work.
