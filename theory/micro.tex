...
\label{sec:micro}
    ...
    \label{subsec:micro:dos}
        ...
        However, real spectra are never perfectly sharp. A simple and very effective phenomenology is the broadening by Dynes parameter\footnote{Throughout this work we distinguish between the Dynes broadening parameter $\gamma$, which enters the quasi-particle density of states in the microscopic tunneling description, and the tunneling rates $\Gamma$ that appear in the stochastic description of incoherent charge transfer. The two quantities are unrelated and refer to different physical mechanisms.}. It is implemented by the substitution $E\to E+\ima\gamma$, while just considering the real part
        \begin{equation}
