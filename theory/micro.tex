...
\label{sec:micro}
    ...
    \label{subsec:micro:pat}
        ...
        Photon-assisted tunneling describes the exchange of photon energies between tunneling charges, such as quasi-particles and the microwave field. Possible energies are given by
        \begin{equation}
        ...
        \end{equation}
        Here $V_0$ is the applied voltage bias, $A$ the amplitude, and $\nu$ the frequency of the microwave field across the junction. The field is assumed to remain unaltered by the tunneling current, meaning no cavity or backaction effects, and the drive is weak enough not to heat the electrodes, allowing both to stay in thermal equilibrium. The voltage couples linearly with the electrostatic potential by $-eV(t)$ and introduces a time-dependent phase to the tunneling amplitude. The model therefore applies to incoherent quasi-particle tunneling and excludes phase-coherent phenomena such as the Josephson supercurrent, which are treated separately in Section~\ref{sec:macro}.

        However, this periodic modulation generates a series of harmonic components in the tunneling amplitude, corresponding to processes in which charges exchange discrete quanta of energy with the oscillating field. When averaged over many cycles, these contributions combine into a stationary current that can be written as a weighted sum of shifted copies of the static \textit{I-V} curve, with each shift by $V_n$ corresponding to the net energy exchange $E_n$. The resulting formula is given by
        \begin{equation}
            I(V_0) = \sum_{n=-\infty}^{\infty} J_n^2\!\left( \frac{eA}{h\nu}\right) \cdot I_0\!\left(V_0 - \frac{n h\nu}{e}\right)\,.
            \label{eq:micro:pat-tien-gordon}
        \end{equation}

        The Bessel functions of the first kind $J_n$ arise from the periodic phase modulation induced by the oscillating potential and quantify the probability for an electron to absorb or emit $n$ photons. Each term in the sum corresponds to a replica of the static $I_0(V_0)$ curve, shifted in voltage by $V_n$ and weighted by $J_n^2(eA/h\nu)$. The resulting current therefore reflects the superposition of all photon-assisted tunneling processes, which manifest as evenly spaced sidebands of the superconducting features in the measured $I(V_0)$.
