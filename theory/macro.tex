...
\label{sec:macro}
    ...
    The GL theory thus provides a macroscopic, phenomenological framework that links the superconducting order parameter to observable quantities such as current and magnetic response. GL provides the macroscopic phase variable whose spatial variation carries current. In Josephson junctions this phase difference becomes the central dynamical variable.
    ...
    \label{subsec:macro:josephson}
        ...
        \begin{wrapfigure}[13]{r}{0.4\textwidth}
            ...
            \caption{Josephson supercurrent and quasi-particle tunneling at $\phi=\pi/9$.}
            \label{fig:macro:josephson-iv}
        \end{wrapfigure} 
        Including quasiparticle tunneling in parallel to the supercurrent produces the characteristic \textit{I--V} curve shown in Figure~\ref{fig:macro:josephson-iv}, with a zero-voltage supercurrent branch and a dissipative branch above the gap.
        ...
        The strength of the phase coupling in a Josephson junction is quantified by the Josephson energy,
        \begin{equation}
            E_\mathrm{J}=\frac{\hbar I_\mathrm{C}}{2e}\,,
        \end{equation}
        which measures the energetic stiffness of the phase and thus the robustness of coherent Cooper-pair tunneling. 
        ...
        \begin{wrapfigure}[13]{r}{0.4\textwidth}
            ...
            \caption{Temperature dependence of the critical current $I_\mathrm{C}(T)$.}
            \label{fig:macro:I_C-suppression}
        \end{wrapfigure} 
        For a tunnel junction, the critical current is determined microscopically by the Ambegaokar--Baratoff (AB) relation,
        \begin{equation}
            I_\mathrm{C}(T)  = \frac{\pi }{2}\ \frac{G_\mathrm{N}\Delta(T)}{e}\ \tanh\!\left(\frac{\Delta(T)}{2 k_\mathrm{B} T}\right)\,
            \label{eq:macro:critical-current}
        \end{equation}
        shown in Figure~\ref{fig:macro:I_C-suppression}. It reflects both the BCS temperature dependence of the gap (Eq.~\ref{eq:micro:DeltaT}) and the thermal population of quasi-particles. At zero temperature, the AB relation reduces to $I_\mathrm{C} = \pi/2\cdot G_\mathrm{N}\Delta_0/e$.
        
        Together with the thermal energy $E_\mathrm{T}=k_\mathrm{B}T$ and the charging energy $E_\mathrm{C}=e^2/2C$, the Josephson energy determines whether the phase behaves classically or undergoes significant thermal or quantum fluctuations. In the regime $E_\mathrm{J}\gg\{E_\mathrm{T},E_\mathrm{C}\}$, the Josephson relations hold in their coherent form, and the junction supports a well-defined supercurrent. In practice, finite temperature and environmental fluctuations lead to thermally activated phase slips, which smear the ideal vertical switching of the Josephson \textit{I--V} characteristic. This rounding is captured in the RCSJ/phase-diffusion framework discussed in Section~\ref{subsec:macro:rcsj}, but does not alter the fundamental Josephson relations introduced here.
    ...
    \label{subsec:macro:shapiro}
        ...
        \begin{equation}
            I_\mathrm{J}(V_0) = \sum_{n=-\infty}^{\infty} (-1)^n J_n\!\left( \frac{2eA}{h\nu}\right) \cdot I_\mathrm{C} \sin(\phi_0 + 2\pi(\nu_0t-n\nu t))\,.
        ...
        \begin{equation}
            I_\mathrm{J}(V_0) = \sum_{n=-\infty}^{\infty} J_n\!\left( \frac{2eA}{h\nu}\right) \cdot I_0\!\left(V_0 - \frac{n h\nu}{2e}\right)\,.
