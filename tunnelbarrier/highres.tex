...
%=========================================================
    ...
    \begin{wrapfigure}[14]{r}{40mm}
        ...
        \vspace{-1.5em}
        \caption{
                SEM image of TB1. The $70\,\mathrm{nm}$ lower Al layer was oxidized in situ at $4\,\mathrm{mbar}$ for $2$:$15\,\mathrm{min}$. The top layer is $92\,\mathrm{nm}$ thick Al, and the barrier overlap is about $50\times100\,\mathrm{nm^2}$.
            }
    ...
    In principle, one static \textit{I--V} fit would define the junction parameters for all later analysis. In practice, the extracted static parameters varied slightly between measurement blocks. I attribute these differences to slow experimental drifts, not to a modified junction. I therefore used a representative static reference fit for each measurement block (M1-4) listed in Table~\ref{table:tunnelbarrier:highres}. Within each measurement block, the corresponding static parameters were then held fixed while the intended calibration parameter was extracted, namely $T_\mathrm{fit}$ for the temperature sweep and the effective microwave amplitude $A_\mathrm{fit}$ for the microwave amplitude sweeps.
    ...
    %=========================================================
        ...
        Here $T_\mathrm{base}$ is the effective temperature floor, $T_\mathrm{c}$ is the upper saturation temperature, $\eta_\mathrm{bath}^{(\mathrm{fit})}$ is the slope relating thermometer and fitted temperature in the intermediate regime, and $T_\mathrm{off}$ is the extrapolated temperature offset between both axes. This procedure avoids manually excluding the plateau and saturation points. The full sweep is described with the same mapping, while the lower and upper bounds encode the weak temperature sensitivity far below and close to $T_\mathrm{c}$. This procedure gives
        \begin{equation}
            \begin{aligned}
        ...
        \begin{figure}[p]
            ...
            \caption{
                Transport landscape of TB1 as a function of temperature on the remapped $T_\mathrm{fit}$ axis. The three columns show the measured \textit{I--V}, \textit{dI--dV}, and \textit{dV--dI} response, while the rows show representative traces, a surface view, and a false-color map of the same data.
            }
    ...
    %=========================================================
        ...
        I next used TB1 to calibrate the microwave amplitude axis and to test the dynamic range of the PAT measurement. The $18.3\,\mathrm{GHz}$ amplitude sweep (M2) established the amplitude calibration. The $13.6\,\mathrm{GHz}$ amplitude sweep (M3) then used the remapped amplitude axis to test how far photon assisted sidebands remained resolvable in the multiphoton regime.
        ...
        \begin{wrapfigure}[14]{r}{0.4\textwidth}
            ...
            \vspace{-0.5em}
            \caption{
                Microwave amplitude calibration of M2. The points give the fitted ac voltage used for remapping, while the slope summarizes the effective coupling.
                }
        ...
        \end{equation}
        The abrupt feature around $A_\mathrm{out}\approx300\,\mathrm{mV}$ is not a change of junction physics. It is consistent with a change in the internal source amplification state of the network analyzer. At the largest source settings the fitted amplitudes also deviated from a linear slope, which I treated as a limit of using the VNA output as a microwave power coordinate.
    ...
    %=========================================================
        ...
        I then used the $13.6\,\mathrm{GHz}$ amplitude sweep (M3) as a dynamic-range test of the same remapped-axis picture. This frequency is favorable for high-order sidebands because the microwave coupling to TB1 was stronger and the photon energy was smaller than for $18.3\,\mathrm{GHz}$. For a comparable nominal source range, both effects increase the reduced amplitude and push spectral weight into higher Bessel sidebands. A representative \text{I--V} is shown in Fig.~\ref{fig:tb:highres:amp13.6GHz:fit}.
        ...
        At the highest source settings, Fig.~\ref{fig:tb:highres:amp13.6GHz:fit} shows a plateau because the VNA reached its maximum available output power. For this fixed contact, the coupling is about a factor of 2.6 larger than the fitted coupling at $18.3\,\mathrm{GHz}$. This comparison demonstrates a strong frequency dependence, but it does not establish a universal frequency response because the coupling may change when the contact configuration changes. The $\eta$ values listed in Table~\ref{table:tunnelbarrier:highres} are therefore summary slopes from these secondary fits, not the calibration inputs used to remap the transport landscapes. Together with the lower photon energy, the larger slope makes the $13.6\,\mathrm{GHz}$ amplitude sweep the high-amplitude benchmark of the section.

        Figures~\ref{fig:tb:highres:amp13.6GHz:cal} and \ref{fig:tb:highres:amp13.6GHz:sim} show the remapped $13.6\,\mathrm{GHz}$ amplitude sweep (M2) and the corresponding simulation. The $2\Delta$ peak acts as a sharp energy marker, and the microwave drive creates many photon shifted copies of this marker. The data resolve these replicas to at least the 25-photon range.
