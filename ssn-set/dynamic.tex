...
%=========================================================
    ...
    \end{align}
    This value was approximately $4\%$ below the resistance quantum for Cooper pairs,
    \begin{align}
        R_\mathrm{Q}^{2e} = \frac{h}{(2e)^2} \approx \qty{6.45}{\kilo\ohm}\,.
        \label{eq:ssn-set:dynamic:cooper-pair-resistance}
    \end{align}
    The relevance of this scale had been suggested before the separate LB and HB fits were performed.\footnote{Lukas Kammermeier first suggested comparing the two resistance scales with $R_\mathrm{Q}^{2e}$ during the preliminary inspection of the data.} Its agreement with the fitted resistance difference was therefore not the result of selecting a comparison only after obtaining the fit values. Nevertheless, the present treatment within \textit{P(E)} Theory did not predict an additive resistance of exactly $R_\mathrm{Q}^{2e}$. I therefore treated this result as a numerical correspondence rather than evidence for a quantized Cooper pair resistance.
