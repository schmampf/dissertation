...
%=========================================================
    ...
    I then partially opened the MCBJ. Unlike SET1, the resulting configuration did not show periodic gate modulation. Its normal-state conductance measured at $T_\mathrm{bath}=\qty{70(1)}{\milli\kelvin}$ was nevertheless suppressed around zero bias, as shown in Fig.~\ref{fig:ssn2-set:dcb}. I fitted the complete measured \textit{I--V} range with the effective RC environment introduced in Sec.~\ref{subsec:stochastic:dcb}. The normal-state resistance, effective resistance, effective capacitance, and effective temperature were free parameters in a single simultaneous fit. The fit reproduced the voltage dependent conductance with
    \begin{align}
    ...
    The agreement between $R_\mathrm{N}^\mathrm{HB}$ and $R_\mathrm{TB}$ had a natural series circuit interpretation. If the superconducting break junction remained on a zero-voltage branch, it could carry the series current while the applied voltage dropped predominantly across the SIN tunnel barrier. As an order-of-magnitude reference, the Ambegaokar-Baratoff relation in the tunneling limit, Eq.~\eqref{eq:macro:critical-current}, evaluated with $R_\mathrm{BJ}$ and $\Delta_0$, gave $I_\mathrm{c}^\mathrm{AB}\approx\qty{150}{\nano\ampere}$. This scale was well above $I_\mathrm{x}=\qty{13(1)}{\nano\ampere}$. Because the break junction was a finite-transmission multichannel contact with an unknown pincode, the Ambegaokar-Baratoff value was not a quantitative critical current prediction. It nevertheless supported a negligible break junction voltage as a plausible explanation for the HB resistance.
    ...
    \end{align}
    This value was approximately $4\%$ below the resistance quantum for Cooper pairs, $R_\mathrm{Q}^{2e}=h/(2e)^2\approx\qty{6.45}{\kilo\ohm}$. The relevance of this scale had been suggested before the separate LB and HB fits were performed.\footnote{Lukas Kammermeier first suggested this comparison during the preliminary inspection of the data.} The present treatment within \textit{P(E)} Theory did not predict an additive resistance of exactly $R_\mathrm{Q}^{2e}$, so I retained the proximity as a numerical observation without interpreting it as a quantized Cooper pair resistance.
    ...
    Taken together, the two devices demonstrated that the same nominal SSN design could produce distinct transport regimes after mechanical opening. SET1 formed a gate controlled island described at a minimal level by orthodox SET theory and later showed gate dependent superconducting switching. SET2 retained a high-conductance contact and developed a gate independent zero bias anomaly consistent with an effective DCB description. The measurements did not identify the microscopic contact properties responsible for these different outcomes. Because each device provided only one usable configuration, the comparison established the observed regimes but not a systematic transition between them.
