...
%%% corporate design %%%
    ...
    % Adds \bookmarksetupnext, used to nest the appendix chapters one level
    % below the "Appendices" umbrella entry in the PDF outline (see
    % miscellaneous/references.tex). themeKonstanz.sty already loads
    % hyperref, so this only adds the bookmark package on top of it.
    \usepackage{bookmark}

    \hypersetup{
        hypertexnames=false,
        pdftitle={Microwave Driven Transport in Mesoscopic Superconducting Junctions from Tunnel Barriers to Atomic Contacts},
        pdfauthor={Oliver Aschenbrenner},
        pdfsubject={Doctoral thesis in experimental condensed matter physics},
        pdfkeywords={superconducting junctions, microwave driven transport, photon assisted tunneling, multiple Andreev reflection, atomic contacts, Josephson effect}
    }
...
%%% bibliography %%%
    ...
    % Compress consecutive citations and format all bibliography-related
    % ranges with a thin-spaced text hyphen, for example [1\,-\,5]. Define
    % this in the language extras because biblatex reapplies them when the
    ...
    % Abbreviate given names to initials across every entry type, so the
    % numbered reference list uses one consistent author-name format
    % regardless of whether an entry is an article, book, thesis, or
    % proceedings contribution.
    \ExecuteBibliographyOptions{giveninits=true}

    % Print URLs for online and software entries, followed by their access dates.
    % Keep URLs hidden for the other entry types unless enabled separately.
...
%%% basic settings %%%
    ...
    % Prevent single opening or closing lines of a paragraph from being
    % stranded at a page break. Include the corresponding case after a
    % displayed equation.
    \clubpenalty=10000
    \widowpenalty=10000
    ...
    % Page style for a chapter's opening page: the chapter title is already
    % set as the large heading on this page, so the running header would
    % otherwise repeat it right above; drop the chapter mark here but keep
    % the page number in its usual place. Apply with \thispagestyle right
    % after \chapter{...} where wanted. The appendix setup applies it as the
    % default opening-page style for appendix chapters.
    ...
    % Keep multi-line footnotes intact on one page instead of splitting them
    % across a page break. Matters most for the digital reading experience.
...
%%% math stuff %%%
    ...
    \let\texthbar\hbar
    \let\hbar\mathhbar
    % Compact comparison symbols. The thesis sans math font renders \ll,
    % \gg, and \lesssim substantially taller than an equals sign. Scale the
    % glyphs while retaining relation spacing and the active math style.
    \newcommand{\scaledmathrel}[2]{%
        \mathrel{\mathchoice
            {\vcenter{\hbox{\scalebox{#1}{$\displaystyle#2$}}}}%
            {\vcenter{\hbox{\scalebox{#1}{$\textstyle#2$}}}}%
            {\vcenter{\hbox{\scalebox{#1}{$\scriptstyle#2$}}}}%
            {\vcenter{\hbox{\scalebox{#1}{$\scriptscriptstyle#2$}}}}}%
    }
    \newcommand{\compactmathrel}[1]{\scaledmathrel{0.78}{#1}}
    \newcommand{\mediummathrel}[1]{\scaledmathrel{0.85}{#1}}
    \newcommand{\sll}{\compactmathrel{\ll}}
    \newcommand{\sgg}{\compactmathrel{\gg}}
    \newcommand{\slesssim}{\compactmathrel{\lesssim}}
    \newcommand{\sgtrsim}{\compactmathrel{\gtrsim}}
    \newcommand{\slt}{\compactmathrel{<}}
    \newcommand{\sgt}{\compactmathrel{>}}
    \newcommand{\sleq}{\compactmathrel{\leq}}
    \newcommand{\sgeq}{\compactmathrel{\geq}}
    \newcommand{\ssim}{\mediummathrel{\sim}}
    \newcommand{\spropto}{\mediummathrel{\propto}}
    \newcommand{\sapprox}{\mediummathrel{\approx}}
    \newcommand{\sequiv}{\mediummathrel{\equiv}}
    \usepackage{textcomp}  % \textmu
    \usepackage{upgreek} % \upmu
    \newfontfamily\thesissanssymbolfamily{lmsans9-regular.otf}
    % XeLaTeX makes siunitx select text-font variants of the micro and ohm
    % symbols at \begin{document}. The legacy thesis font encoding does not
    % provide them reliably. Use the optical-size body sans font in prose and
    % upright serif math Greek in equations. Apply the same context-sensitive
    % treatment to the ohm symbol.
    \AtBeginDocument{%
        \DeclareSIPrefix\micro{\ifmmode\upmu\else{\thesissanssymbolfamily µ}\fi}{-6}%
        \DeclareSIUnit\ohm{\ifmmode\Omega\else{\thesissanssymbolfamily Ω}\fi}%
